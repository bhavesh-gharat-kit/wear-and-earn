...
export async function handlePaidJoining(tx, order) {
  ...
  // Calculate total commission amount (in paisa)
  const commissionInPaisa = Math.floor(product.commissionAmount);
  const totalCommission = commissionInPaisa * (order.quantity || 1);
  
  // Company cut: 30% of commission
