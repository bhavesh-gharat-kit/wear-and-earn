...
export const sendRegistrationOTP = async (email, contactNo, domainForWebOtp) => {
  if (!email && !contactNo) return false;

  // create OTP, hash it and set expiry
  const otp = Math.floor(100000 + Math.random() * 900000).toString();
  console.log("🔢 Generated Registration OTP:", { otp, type: typeof otp, length: otp.length });
  const otpHash = await bcrypt.hash(otp, 10);
  console.log("🔒 Registration OTP Hash created:", { hashPreview: otpHash.substring(0, 10) + "..." });
  const expiresAt = new Date(Date.now() + 10 * 60 * 1000); // 10 minutes

  try {
    const title = `Registration OTP - ${process.env.NEXT_PUBLIC_COMPANY_NAME}`;
    // store OTP in database
    console.log("💾 Storing registration OTP in database:", { contact: contactNo, email, expiresAt });
    ...
    console.log("✅ Registration OTP stored successfully:", { id: otpRecord.id, contact: otpRecord.contact, email: otpRecord.email });
    ...
    if (contactNo) {
      try {
        await sendFast2Sms(contactNo, otp, { domainForWebOtp });
      } catch (smsError) {
...
export const sendPasswordResetOTP = async (email, contactNo, domainForWebOtp) => {
  if (!email && !contactNo) return false;
...
export const verifyOTP = async (email, contactNo, otp, deleteAfterVerify = true) => {
  try {
    ...
    // Generate alternative mobile number formats for searching
    const mobileSearchTerms = [];
    if (contactNo) {
      mobileSearchTerms.push(contactNo); // Original format
      
      // Add cleaned version
      let cleanMobileNo = contactNo.toString().trim().replace(/\s+/g, '');
      ...
      if (cleanMobileNo !== contactNo) {
        mobileSearchTerms.push(cleanMobileNo);
      }
      
      // Add with +91 prefix if not already present
      if (!contactNo.startsWith('+91') && !contactNo.startsWith('91')) {
        mobileSearchTerms.push(`+91${contactNo}`);
        mobileSearchTerms.push(`91${contactNo}`);
      }
    }
    
    console.log("📱 Searching for mobile numbers:", mobileSearchTerms);
    
    // Debug: Check all OTPs in database first
    ...
    console.log("🔍 All active OTPs in database:", allOTPs.map(otp => ({
      id: otp.id,
      email: otp.email,
      contact: otp.contact,
    ...
    // Find the latest OTP for this email/contact with multiple mobile formats
    const otpRecord = await prisma.OTP.findFirst({
      where: {
        OR: [
          ...(email ? [{ email: email }] : []),
          ...mobileSearchTerms.map(mobile => ({ contact: mobile }))
        ],
        expiresAt: {
          gte: new Date()
        }
    ...
    console.log("🔐 Comparing OTP:", { 
      ...
      stored: "HASHED",
      storedHash: otpRecord.otpHash.substring(0, 10) + "..." 
    });
    
    // Ensure OTP is string for comparison
    const otpString = String(otp);
    const isValidOTP = await bcrypt.compare(otpString, otpRecord.otpHash);
    
    console.log("✅ OTP validation result:", { 
      isValid: isValidOTP,
      comparedString: otpString,
      originalOtp: otp 
    });
...
const sendFast2Sms = async (numbers, otp, options = {}) => {
  ...
  // Create WebOTP compatible message with proper format
  const domain = (options.domainForWebOtp?.replace(/https?:\/\//, '') || process.env.NEXT_PUBLIC_SITE_URL?.replace(/https?:\/\//, '') || 'wear-and-earn.vercel.app').trim();
  
  // Generate random hash for security (WebOTP recommendation)
  const hash = Math.random().toString(36).substring(2, 8);
  
  // Correct WebOTP format: <#> AppName code: OTP \n @domain #hash
  const webOTPMessage = `<#> Your verification code is: ${otp}\n@${domain} #${hash}`;
  console.log("✉️ Using domain for WebOTP SMS:", domain);
  console.log("📱 SMS message format:", webOTPMessage);

  const params = new URLSearchParams({
    authorization: process.env.FAST2SMS_API_KEY,
    route: "q", // Use quick route for custom message
    message: webOTPMessage,
