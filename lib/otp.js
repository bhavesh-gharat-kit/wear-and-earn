...
export const sendRegistrationOTP = async (email, contactNo) => {
  ...
  try {
    const title = `Registration OTP - ${process.env.NEXT_PUBLIC_COMPANY_NAME || "Wear and Earn"}`;
    ...
    <body>
        <div class="container">
            ...
            <p class="note">⚠️ Please do not share this OTP with anyone. It is valid for <b>5 minutes</b> only.</p>
...
export const sendPasswordResetOTP = async (email, contactNo) => {
  ...
  try {
    const title = `Password Reset OTP - ${process.env.NEXT_PUBLIC_COMPANY_NAME || "Wear and Earn"}`;
    ...
    if (email) {
      ...
      <body>
          <div class="container">
              ...
              <div class="otp">${otp}</div>
              <p class="note">⚠️ Do not share this OTP with anyone. It will expire in <b>5 minutes</b>.</p>
...
const sendBrevoEmail = async (recepient, subject, htmlContent) => {
  const emailOptions = {
    sender: {
  ...
  try {
    ...
    if (!res.ok) {
      const errorData = await res.text(); // capture raw response
      console.error("❌ Brevo API error:", res.status, errorData);
      throw new Error(`Brevo API failed with status ${res.status}`);
    }
