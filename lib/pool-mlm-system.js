...
async function updateTeamFormation(tx, userId) {
  ...
  console.log(`👥 User ${userId} sponsor: ${user?.sponsorId}`);
  ...
  console.log(`🔍 Existing team for sponsor ${user.sponsorId}:`, team ? `Team ID: ${team.id}, Members: ${team.members.length}` : 'No team found');
  ...
  console.log(`📊 Team ${team.id} new size: ${newTeamSize}/${MLM_CONFIG.TEAM_SIZE}`);
  
  await tx.team.update({
    where: { id: team.id },
    data: {
      status: newTeamSize >= MLM_CONFIG.TEAM_SIZE ? 'COMPLETE' : 'FORMING',
      completedAt: newTeamSize >= MLM_CONFIG.TEAM_SIZE ? new Date() : null
    }
  });
  
  console.log(`🏆 Team ${team.id} status: ${newTeamSize >= MLM_CONFIG.TEAM_SIZE ? 'COMPLETE' : 'FORMING'}`);
