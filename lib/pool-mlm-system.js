...
const MLM_CONFIG = {
  ...
  // Withdrawal limits
  MIN_WITHDRAWAL_AMOUNT: 30000, // ₹300 in paisa (consistent currency unit)
};
...
export async function processPoolMLMOrder(tx, order) {
  ...
  try {
    ...
    // Generate referral code for first-time purchasers
    ...
    // Always generate referral code if user doesn't have one and this contains first purchase
    if (hasFirstPurchase) {
...
async function addToTurnoverPool(tx, amount) {
  ...
  console.log(`📈 Added ₹${amount/100} to turnover pool`);
}
...
async function generateReferralCodeIfNeeded(tx, userId) {
  ...
  console.log(`🎟️ Generated referral code ${referralCode} for user ${userId}`);
