      ...
      for (let i = 0; i < levelUsers.length; i++) {
        ...
        await tx.wallet.create({
          data: {
            ...
            amount: userAmount,
            status: 'completed',
            reference: `pool_dist_${new Date().getTime()}`,
            description: `Level L${level} pool distribution - ₹${(userAmount/100).toFixed(2)}`
          }
        });
        
        // Update user wallet balance and total pool income
        await tx.user.update({
          where: { id: user.id },
          data: {
            walletBalance: { increment: userAmount },
            totalPoolIncomeEarned: { increment: userAmount/100 } // Convert to rupees for tracking
          }
        });
