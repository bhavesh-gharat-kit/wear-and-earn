...
const MLM_CONFIG = {
  ...
  COMPANY_SHARE: 0.25,  // 25% to company
  POOL_SHARE: 0.75,     // 75% for MLM pool and self income
  ...
  // 20% commission to direct sponsor instead of weekly self infome(also in repurchase)
  DIRECT_SPONSOR_PERCENTAGE: 0.20,
...
export async function processPoolMLMOrder(tx, order) {
  ...
  try {
    ...
    for (const purchase of purchases) {
      ...
      // Direct sponsor income regardless first purchase(discussed on 18/11/2025)
      // if (purchase.isFirstPurchase) {
      // First Purchase: 20% sponsor income + 80% to turnover pool
...
async function processFirstPurchase(tx, purchase, poolShare) {
  ...
  // Calculate direct sponsor income and turnover pool amounts
  const directSponsorIncome = Math.floor(poolShare * MLM_CONFIG.DIRECT_SPONSOR_PERCENTAGE);
  const turnoverPoolAmount = poolShare - directSponsorIncome;
  ...
  if (directSponsorId) {
    // Income to direct sponsor
    const walletTxn = await tx.Wallet.create({
      data: {
        userId: directSponsorId,
        type: 'direct_sponsor_income',
        amount: directSponsorIncome,
        status: 'completed',
        reference: `direct_sponsor_income_${purchase?.id}`,
        description: `Direct sponsor income for order id: ${purchase?.id}`
      }
    })
    await tx.User.update({
      ...
      data:{
        walletBalance: {
          increment: directSponsorIncome
        }
      }
    })
  } else {
    // If direct sponsor not found then rollup to company
  }
