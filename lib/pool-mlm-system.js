...
const MLM_CONFIG = {
  ...
  // Withdrawal limits
  MIN_WITHDRAWAL_AMOUNT: 30000, // ₹300 in paisa
};
...
export async function processPoolMLMOrder(tx, order) {
  ...
  try {
    ...
    // Update user referral code if first purchase and not yet set
    for (const purchase of purchases) {
      if (purchase.isFirstPurchase) {
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    for (let level = 1; level <= 5; level++) {
      ...
      for (let i = 0; i < levelUsers.length; i++) {
        ...
        const userAmount = perUserAmount + (i === 0 ? remainderAmount : 0); // Give remainder to first user
