...
class MLMCompatibilityBridge {
  constructor() {
    this.prisma = prisma
  } SYSTEM COMPATIBILITY BRIDGE - OPTION B
...
class MLMCompatibilityBridge {
    ...
    try {
      ...
      if (newLevel > currentLevel) {
        ...
        await this.prisma.referralTracking.create({
          data: {
            referrerId: userId,
            referredUserId: userId, // Self-reference for level promotion
            referralDate: new Date(),
            firstPurchaseCompleted: true,
            teamContributionStatus: `level_promotion_L${newLevel}`,
            referralCodeUsed: `LEVEL_UP_${currentLevel}_TO_${newLevel}`
          }
        })
    ...
    try {
      const result = await this.prisma.$transaction(async (tx) => {
        ...
        await tx.referralTracking.create({
          data: {
            referrerId: userId,
            referredUserId: userId, // Self-reference for purchase tracking
            referralDate: new Date(),
            firstPurchaseCompleted: true,
            firstPurchaseDate: new Date(),
            teamContributionStatus: 'purchase_completed',
            referralCodeUsed: `MLM_PURCHASE_${orderData.mlmPrice}`
          }
        })
  ...
  async disconnect() {
    // No manual disconnect needed with singleton Prisma client
  }
}
