...
export async function activateUserInMLM(userId, orderId, tx = prisma) {
  ...
  // Update hierarchy (add relationships up to 5 levels)
  const uplines = await getUplines(slot.parentId, 5)
...
export async function distributeJoiningCommission(userId, orderId, commissionAmount, tx = prisma) {
  ...
  // Company gets 30% (updated from 25%)
  const companyShare = Math.round(commissionInPaisa * 0.30)
  ...
  // Remaining 70% split: 70% to uplines, 30% to self (weekly)
  ...
  // Distribute to uplines (70% of 75%) - 5 levels
  const uplines = await getUplines(userId, 5)
  const levelPercentages = [0.30, 0.25, 0.20, 0.15, 0.10] // Total = 100% of upline share (5 levels)
  
  for (let i = 0; i < uplines.length && i < 5; i++) {
...
export async function distributeRepurchaseCommission(userId, orderId, commissionAmount, tx = prisma) {
  ...
  // Distribute 70% to uplines with repurchase percentages - 5 levels
  const uplineShare = commissionInPaisa - companyShare
  const uplines = await getUplines(userId, 5)
  const levelPercentages = [0.35, 0.25, 0.20, 0.15, 0.05] // Total = 100% of upline share (5 levels)
