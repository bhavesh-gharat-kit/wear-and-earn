...
export async function handlePaidJoining(tx, order) {
  ...
  if (order.orderProducts && order.orderProducts.length > 0) {
    totalCommissionAmount = order.orderProducts.reduce((sum, orderProduct) => {
      // Use mlmPrice if available, fallback to legacy commissionAmount, then to 0
      const commissionPerUnit = orderProduct.product?.mlmPrice || orderProduct.product?.commissionAmount || 0;
