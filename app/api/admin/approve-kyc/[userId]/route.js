...
export async function POST(request, { params }) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (user.isKycApproved && action === 'approve') {
        throw new Error('User KYC is already approved')
      }

      // Get current KYC status
      const currentKycStatus = user.kycDocument.status
      ...
      const updatedUser = await tx.user.update({
        ...
        data: {
          ...
          kycApprovedBy: action === 'approve' ? session.user.id : null
        },
        include: {
          kycDocument: true
        }
      ...
      const updatedKycDocument = await tx.kycDocument.update({
        where: { userId: userId },
        data: {
      ...
      await tx.adminAction.create({
        data: {
          ...
          details: {
            ...
            reason: reason,
            rejectionReasons: action === 'reject' ? rejectionReasons : null,
            previousStatus: currentKycStatus,
            newStatus: action === 'approve' ? 'approved' : 'rejected',
            kycDocumentId: user.kycDocument.id
          },
      ...
      await tx.ledger.create({
        data: {
          ...
          type: `kyc_${action}`,
          amount: 0,
          description: `KYC ${action} by admin`,
      ...
      return {
        user: updatedUser,
        kycDocument: updatedKycDocument,
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        kycStatus: {
          isApproved: result.user.isKycApproved,
          status: result.kycDocument.status,
          approvedAt: result.user.kycApprovedAt,
          reviewedAt: result.kycDocument.reviewedAt,
