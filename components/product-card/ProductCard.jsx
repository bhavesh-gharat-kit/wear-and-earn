...
function ProductCard({ product, variant = "grid", compact = false }) {
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg sm:rounded-xl border border-gray-200 dark:border-gray-700 p-3 sm:p-6 hover:shadow-md transition-all duration-200 ring-2 ring-purple-500/70 shadow-[0_0_16px_4px_rgba(139,92,246,0.25)]">
        <div className="flex gap-3 sm:gap-6">
          ...
          <div className="relative flex-shrink-0">
            <div className="w-20 h-20 sm:w-24 sm:h-24 md:w-32 md:h-32 relative rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-800">
              <Image
                src={productImage}
                alt={title}
                fill
  ...
  return (
    <div className={`bg-white dark:bg-gray-900 rounded-lg sm:rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-lg transition-all duration-300 hover:-translate-y-1 group ring-2 ring-purple-500/70 shadow-[0_0_16px_4px_rgba(139,92,246,0.25)] ${compact ? 'max-w-[180px] mx-auto' : ''}`}>
      {/* Compact Mobile Product Image */}
      <div className={`relative ${compact ? 'aspect-square' : 'aspect-square sm:aspect-[4/5]'} overflow-hidden bg-gray-100 dark:bg-gray-800`}>
        <Image
        ...
        <div className={`absolute ${compact ? 'top-1 left-1' : 'top-2 sm:top-3 left-2 sm:left-3'} flex flex-col gap-1 sm:gap-2`}>
          {discount && (
            <span className={`bg-red-500 text-white ${compact ? 'text-xs py-0.5 px-1' : 'text-xs py-0.5 px-1.5 sm:py-1 sm:px-2'} rounded-full font-medium shadow-sm`}>
              -{discount}%
            </span>
          )}
          {!inStock && (
  ...
  <div className="absolute inset-0 bg-black/20 dark:bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
      ...
      <div className={compact ? "p-2" : "p-3 sm:p-4"}>
        <div className={compact ? "mb-1.5" : "mb-2 sm:mb-3"}>
          ...
          <p className={`${compact ? 'text-xs' : 'text-xs sm:text-sm'} text-gray-500 dark:text-gray-400 truncate`}>
            {category?.name || 'Category'}
          </p>
        </div>
        ...
        <div className={compact ? "mb-2" : "mb-3 sm:mb-4"}>
          <div className="flex flex-col gap-0.5">
            ...
            {price && price !== finalAmount && (
              <span className={`${compact ? 'text-xs' : 'text-xs sm:text-sm'} text-gray-500 dark:text-gray-400 line-through`}>
                ₹{Number(price).toLocaleString("en-IN")}
              </span>
            )}
          </div>
        ...
        <div className={compact ? "space-y-1" : "space-y-1.5 sm:space-y-2"}>
          ...
          <button
            ...
            className={`w-full flex items-center justify-center ${compact ? 'gap-1 py-1.5 px-2 text-xs' : 'gap-1.5 sm:gap-2 py-2 sm:py-2.5 px-3 sm:px-4 text-sm sm:text-base'} bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-100 rounded-lg transition-colors font-medium active:scale-95`}
          >
