...
// import { FaTimes, FaUser, FaMapMarkerAlt, FaBox, FaRupeeSign, FaCalendarAlt, FaPhone, FaEnvelope, FaDownload, FaWhatsapp } from "react-icons/fa";
...
console.log(imageUrl);
  ...
  const handleShareToWhatsApp = async () => {
    ...
    try {
      ...
      const fileName = `Order_Label_${order.id}.pdf`;
      ...
      const customerPhone = order.user?.mobileNo || 'N/A';
...
${order.address || 'No address provided'}`;
      ...
      if (navigator.share && navigator.canShare) {
        ...
        if (canShareFiles) {
          try {
            ...
            console.warn('File share failed, trying text-only share:', shareError);
          }
        }
        ...
        try {
          ...
          toast.success('Shared (text only - PDF not supported on this device)');
          setIsSharing(false);
          return;
      ...
      // Note: WhatsApp wa.me links cannot include file attachments
      const whatsappMessage = `${message}

⚠️ *Note:* Please download the order label from the admin panel to get the PDF.`;

      const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(whatsappMessage)}`;
      window.open(whatsappUrl, '_blank');

      toast.success('WhatsApp opened (PDF sharing not available on desktop - text only)', {
        duration: 4000
      });

    } catch (error) {
      console.error('Share failed:', error);
      toast.error('Failed to share. Please try downloading the label instead.');
    } finally {
      setIsSharing(false);
    }
  };
...
<Image
    ...
    />
        ...
        <div className="sticky bottom-0 bg-gray-50 dark:bg-gray-700 px-6 py-4 border-t border-gray-200 dark:border-gray-600">
          ...
          {/* Info text about sharing limitations */}
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 text-center">
            📱 PDF file sharing works best on mobile devices. Desktop will share text summary only.
          </p>
        </div>
      </div>
    </div>
