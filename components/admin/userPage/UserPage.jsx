...
function UserPage() {
  ...
  const fetchAllUserDetails = async (page = currentPage, rowsPerPage = rowsPage) => {
    const skip = (page - 1) * rowsPerPage;
    try {
      setIsLoading(true);
      const response = await axios.get(`/api/admin/users?limit=${rowsPerPage}&skip=${skip}&searchTerm=${searchTerm}`);
      ...
      toast.error("Internal server error while fetching users");
      setAllUserDetails([]);
    } finally {
      setIsLoading(false);
    }
      ...
      (t) => (
        <div className="space-y-3">
          ...
          <div className="flex justify-start items-center gap-2 flex-row-reverse">
            <button
              className="btn bg-green-600 text-white rounded-md px-3 py-1"
              onClick={() => {
            ...
            <button
              className="btn bg-rose-600 text-white rounded-md px-3 py-1"
  ...
  const handleUserDeleteById = async (id) => {
    try {
      ...
      toast.error("Internal server error while deleting user");
    }
  };
  ...
  // Prevent default search submit
  const handleSubmitSearchUserQuery = (e) => {
    e.preventDefault();
  };

  // Render loader
  if (isLoading) return <LoaderEffect />;

  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex justify-between items-center">
        <h1 className="text-xl text-blue-700 dark:text-blue-400 font-bold">
          Manage Users
        </h1>
      </div>
              ...
              allUserDetails.map((user, index) => (
                <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                  <td className="p-2 sm:p-3 text-xs sm:text-sm">
                    {(currentPage - 1) * rowsPage + index + 1}
                  </td>
                  <td className="p-2 sm:p-3 text-xs sm:text-sm">
                    <div className="font-medium">{user.fullName || "Not Defined"}</div>
                    {/* <div className="text-gray-500 dark:text-gray-400 sm:hidden text-xs">
                      {user.email}
                    </div> */}
                  </td>
