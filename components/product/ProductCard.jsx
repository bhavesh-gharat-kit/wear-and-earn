...
function ProductCard({ product, variant = "default", setShowModal }) {
  ...
  // For slider/display: show sellingPrice - discount (no GST yet)
  ...
  <div className="flex flex-col text-center border rounded-lg overflow-hidden group bg-white dark:bg-gray-900 border-t-4 border-purple-500 dark:border-purple-400 ring-2 ring-purple-500/70 shadow-[0_0_16px_4px_rgba(139,92,246,0.25)]">
          ...
          <div className="relative group">
            ...
            <Image
              ...
              className="w-full h-56 object-cover bg-gray-50 dark:bg-gray-800"
              loading="lazy"
            />

            {/* Product Overlay (Appears on hover) */}
            <div className="absolute inset-0 bg-black/10 dark:bg-black/40 bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2">
                <Link
                  href={`/product-details/${id}`}
                  className="bg-white dark:bg-gray-900 text-black dark:text-gray-100 py-2 px-4 rounded hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
                >
                  Quick View
                </Link>
              </div>
          ...
          <div className="p-4">
            <Link href={`/product-details/${id}`}>
              <h3 className="text-lg font-medium text-gray-800 dark:text-gray-100 mb-2 line-clamp-2 hover:text-blue-600 dark:hover:text-blue-400">
                {title}
              </h3>
            </Link>
  ...
  <div className="group/card card h-full bg-base-100 dark:bg-gray-900 w-72 max-sm:w-full border-t-4 border-purple-500 dark:border-purple-400 hover:border-blue-200 dark:hover:border-blue-400 transition-all duration-200 hover:-translate-y-0.5 ring-2 ring-purple-500/70 shadow-[0_0_16px_4px_rgba(139,92,246,0.25)]">
      ...
      <div className="card-body p-3 max-sm:p-2">
        <h2 className="card-title">
          <Link href={`/product-details/${id}`}>
            <p className="font-medium text-slate-800 dark:text-gray-100 line-clamp-2 max-sm:text-sm hover:text-blue-600 dark:hover:text-blue-400">
              {title}
            </p>
          </Link>
        </h2>
        ...
        <div className="flex justify-start items-center mb-3 gap-2">
          <span className="text-blue-600 dark:text-blue-400 text-xl font-semibold max-sm:text-base">
            ₹{Number(finalAmount).toLocaleString("en-IN")}
          </span>
          {price && price !== finalAmount && (
            <span className="text-gray-500 dark:text-gray-400 text-sm line-through">
